...
## Login into the POD and check the volume mountPath
...
## Now, I am going to delete the deployment, which will delete the POD. After that delete the PVC. Check if our file is still exsist on NFS server?
...
```
### Check the status of pv,pvc. Observe the "status" column and "Reclaim Policy" column.

```
kubectl get pv,pvc
```

### Now, check file that we created earlier on NFS server. File should be there.
...
## Now, delete the PV and check if file is still there?
...
## From the above lab, it is clear that if we set the "Reclaim Policy" value to retain, our file will always available.



## If we deploy same PV,PVC and Deployment, we will observe same file in the POD.
```
[root@master1 volume]# kubectl apply -f LAB-Static-NFS-PV-PVC-Deployment.yaml
...
[root@master1 volume]# kubectl get pods 
NAME                                 READY   STATUS    RESTARTS   AGE
static-deployment1-88b7d5d8c-m7ztz   1/1     Running   0          12s
...
```
kubectl delete -f LAB-Static-NFS-PV-PVC-Deployment.yaml
rm -f LAB-Static-NFS-PV-PVC-Deployment.yaml
```
## clear the lab from NFS server
rm -f static-file1.txt 
```
